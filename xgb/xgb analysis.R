...
# Load libraries ----------------------------------------------------------
require(pacman)
pacman::p_load(tidyverse, xgboost, readxl, hrbrthemes, caret, naniar, glue, fs, sf, rnaturalearthdata, rnaturalearth, rmapshaper, fs, tidytext)
...
xgb.model.function <- function(level, run, cv_folds, cv_repeats, min_rsamp, tlength){
  ...
  data_model <- dbfao_level %>%
    select(df_ha, pop_growth, food_imports, food_exports, foreing_invest,
  ...
  model_var_importance <- 
    ...
    transmute(imp_pop_growth = ifelse(exists("imp_pop_growth"), imp_pop_growth, 0),
              ...
              imp_agricultural_land = ifelse(exists("imp_agricultural_land"), imp_agricultural_land, 0),
              imp_food_inflation = ifelse(exists("imp_food_inflation"), imp_food_inflation, 0)
    )
...
# Load data ---------------------------------------------------------------
dbfao <- read_excel('tbl/dabase_global_total.xlsx')[,-1]
path_output <- './out/run_3'
varbl <- c('df_ha', 'gdp_perc', 'food_exports', 'exports_gdp', 'food_infl',
           'pop_growth', 'gdp_growth', 'foreing_invest', 'tmp_change')

wrld <- ne_countries(returnclass = 'sf', scale = 50) %>% dplyr::select(sov_a3, admin, region_un, geometry)
wrld_tble <- as_tibble(st_drop_geometry(wrld))
dbfao <- inner_join(dbfao, wrld_tble, by = c('iso' = 'sov_a3'))
colnames(dbfao)[2] <- 'iso3'
colnames(dbfao)[12] <- 'continent'
colnames(dbfao)[10] <- 'df_ha'
...
glb <- "glb"
dbfao <- filter(dbfao, year != 2021)

vis_miss(dbfao)
tail(dbfao)
n_miss(dbfao)
miss_case_table(dbfao)
prop_miss_var(dbfao)
miss_var_summary(dbfao)

dbfao[is.na(dbfao$df_ha),] %>% View()
dbfao <- dbfao[!is.na(dbfao$df_ha),]
