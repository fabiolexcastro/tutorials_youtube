...
# Load libraries ----------------------------------------------------------
require(pacman)
p_load(raster, terra, sf, tidyverse, leaflet, classInt, plotly, rnaturalearthdata, RColorBrewer, rmapshaper, geodata, rnaturalearth, gtools, glue, htmltools, mapview, leaflet.extras, fs)
...
pal <- colorNumeric(
      ...
      domain = shpf$total_2017)

pal <- colorNumeric('OrRd', domain = shpf$total_2017)
pal <- colorNumeric(palette = 'YlGnBu', domain = shpf$total_2017)
qpal <- colorQuantile("YlOrRd", shpf$total_2017, n = 7)
...
leaflet() %>% 
    ...
    addPolygons(data = shpf,
                ...
                color = 'white', 
                weight = 0.4, 
                label = ~shpf$total_2017,
                fillColor =  ~pal(shpf$total_2017), #colorNumeric(brewer.pal(name = 'YlOrRd', n = 9), shpf$total_2017) 
                fillOpacity = 0.5,
                smoothFactor = 0.5,
                popup = paste0('Número de habiantes: ', shpf$total_2017, '<br>', 'Municipio:', ' ', str_to_title(shpf$MPIO_CNMBR), '<br>')) %>%
    addLegend('bottomright',
              values = shpf$total_2017,
              labels = shpf$total_2017,
              # labFormat = labelFormat(suffix = 'miles'),
              pal = pal,
              title = 'Cantidad de habitantes') 
